...
int mexp(double *A, double **X, int n){
   ...
   // Continues below
    ...
    for(int i = 0; i < n; i++) {
        for(int j = 0; j < n; j++) {
            temp4[i*n + j] += A2[i*n + j] * 1187353796428800.0 + A4[i*n + j] * 10559470521600.0 + A6[i*n + j]*33522128640.0;
            temp1[i*n + j] += A2[i*n + j] * 7771770303897600.0 + A4[i*n + j] * 129060195264000.0 + A6[i*n + j]*670442572800.0;
        }
